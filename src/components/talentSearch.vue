<template>
  <el-main>
    ...
    <div v-if="userTypeId == 1"
      ...
      <div class="form_bottom">
        <div class="form-item1">
          <label>人才关键字：</label>
          <select v-model="searchData.talentSelect"
                  @change="getTalent">
        ...
        <div class="form-item2">
          ...
          <select v-model="searchData.personalCharacter">
            <option>全部</option>
            <option v-for="(item, index) in personalCharacterDate"
                    :key="index"
        ...
        <div class="form-item">
          <button class="searchBtnTwo"
                  @click="reset">重置搜索数据</button>
        </div>
      </div>
    </div>
    <!--v-if="userTypeId == 1"-->
    <div v-if="userTypeId == 8 || userTypeId == 9"
         class="formBox">
      <div class="form_top">
        <div class="class">
          <label>选择班级：</label>
          <select v-model="searchData.editorial"
          ...
          <select v-model="searchData.pid"
                  @change="changezhuanye($event)">
            <option value="">全部专业</option>
            <option v-for="item in professional"
                    :key="item.id"
                    :value="item.id">{{ item.proname }}</option>
          </select>
          <select style="width: 160px"
                  v-model="ProductActive"
                  @change="changeProduct($event)">
            <option value="">全部班级</option>
            <option v-for="item in className"
                    :key="item.id"
                    :value="item.id">{{
              item.classname
            }}</option>
    ...
    <div class="table">
      <p class="tableTit"
         v-if="INP == ''">
        人才数量:<span class="blue">{{ talents }}</span>位
      </p>
      <p class="tableTit"
         v-else>
        具备‘{{ INP }}’标签人才数为:<span class="blue">{{ talents }}</span>位
      </p>
      ...
      <el-table v-loading="loading"
        ...
        <el-table-column v-if="selectTalent == true"
                         align="center"
                         prop="integralValue1"
        ...
        <el-table-column align="center"
          ...
          <template slot-scope="scope">
            <p class="lookBtn"
               @click="toLookDetail(scope.row)">查看详情</p>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    getTableData() {
      ...
      this.loading = true;
      this.$ajax
        .get(this.baseUrl + talentPortrait, {
          headers: {
    ...
    toLookDetail(row) {
      ...
      let routeData = this.$router.resolve({
        ...
        query: { classId: row.classid, userId: row.id }
      });
    ...
    synchronization() {
      this.$ajax
        .get(this.baseUrl + getsynchronization, {
          params: {
            schoolId: localStorage.getItem("schoolId"),
